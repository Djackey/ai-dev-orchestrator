...
Require `IMPLEMENTATION_SPEC`: **objective, abstract lane, files, interfaces, constraints, verification command, reasoning effort**. Resolve supporting files through `${CLAUDE_PLUGIN_ROOT}`: `${CLAUDE_PLUGIN_ROOT}/contracts/IMPLEMENTATION_SPEC.md` and `${CLAUDE_PLUGIN_ROOT}/contracts/IMPLEMENTATION_LANE_CONTRACT.md`. If those files are unavailable, the self-contained invariants here remain mandatory. Evidence-first domains also require the architect's recorded `EVIDENCE_GATE_SATISFIED`. For an incident, defect, or debugging task that gate rests on `ROOT_CAUSE_CONFIRMED` or an explicit bounded risk acceptance; for a proactive high-risk change there is no defect and no root cause to demand. Missing content is a gap, not permission to invent authority.
